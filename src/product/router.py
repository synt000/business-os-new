...
class BranchCreateInboundSchema(BaseModel):
    branch_name: str
    location_address: Optional[str] = None

class SupplierCreateInboundSchema(BaseModel):
    supplier_name: str
    contact_phone: Optional[str] = None

class ProcurementCreateInboundSchema(BaseModel):
    product_id: str
    supplier_id: str
    qty_purchased: int
    unit_cost: float
...
async def ingest_isolated_omnichannel_order(payload: OrderCreateInboundSchema, request: Request, current_user: User = Depends(require_active_subscription()), db: Session = Depends(get_db)):
    ...
    if tenant and current_orders >= tenant.max_order_limit:
        raise HTTPException(status_code=403, detail=f"SUBSCRIPTION_LIMIT_EXCEEDED: Your current tier allows maximum {tenant.max_order_limit} Orders. Upgrade required.")
    ...
    new_order = Order(order_number=order_num, customer_id=customer.id, platform_channel=payload.platform_channel, customer_name=customer.customer_name, customer_phone=customer.customer_phone, total_amount=0.0, order_status="PENDING", tenant_id=current_user.tenant_id)
    db.add(new_order)
    db.flush() # Secure new_order.id dynamically generated by UUIDv4 Shard
    ...
    for item in payload.items:
        ...
        new_item = OrderItem(quantity=item.quantity, price_at_sale=sale_price, order_id=new_order.id, product_id=target_product.id)
        db.add(new_item)
        
    new_order.total_amount = computed_total_pool
    
    # GAAP Double-Entry Alignment Protocols
    record_double_entry_accounting(db, current_user.tenant_id, "DEBIT", "CASH_ASSET", computed_total_pool, new_order.id, f"Inbound cash received from sales invoice {order_num}")
    ...
    try:
        customer.total_spent += computed_total_pool
        db.commit()
...
async def fetch_tenant_subscription_usage_meters(current_user: User = Depends(require_active_subscription()), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "feature_flags": {"multi_branch_pos_enabled": tenant.enable_pos_feature, "ai_predictive_forecast_enabled": tenant.enable_ai_forecast}
    }
...
@router.get("/branches")
async def fetch_all_isolated_branches(current_user: User = Depends(require_active_subscription()), db: Session = Depends(get_db)):
...
async def create_procurement_purchase_entry(payload: ProcurementCreateInboundSchema, request: Request, current_user: User = Depends(require_active_subscription()), db: Session = Depends(get_db)):
    ...
    record_double_entry_accounting(db, current_user.tenant_id, "DEBIT", "INVENTORY_ASSET", total_cost_pool, new_po.id, f"Purchased inventory asset inflow via {po_number}")
    record_double_entry_accounting(db, current_user.tenant_id, "CREDIT", "CASH_ASSET", total_cost_pool, new_po.id, f"Cash outflow payment remitted for purchase invoice {po_number}")
    ...
    return {"status": "PROCUREMENT_PURCHASE_SUCCESSFULLY_LOGGED", "procurement_id": new_po.id, "po_number": po_number, "total_cost": total_cost_pool}
...
async def create_isolated_crm_customer(payload: CustomerCreateInboundSchema, request: Request, current_user: User = Depends(require_active_subscription()), db: Session = Depends(get_db)):
    ...
    db.add(new_customer)
    log_security_audit_action(db, current_user.id, current_user.tenant_id, "CREATE", "CRM_CUSTOMERS", f"Registered CRM Client: {payload.customer_name}", request)
    db.commit()
...
async def create_secure_workspace_invitation(payload: WorkspaceInviteInboundSchema, request: Request, current_user: User = Depends(require_active_subscription()), db: Session = Depends(get_db)):
    ...
    new_invite = WorkspaceInvitation(invite_email=payload.invite_email, target_role=payload.target_role, invitation_token=secure_token, expires_at=expiry_time, tenant_id=current_user.tenant_id)
    db.add(new_invite)
...
async def pay_order(
    ...
    if payment_method not in ["CASH", "KBZPAY", "WAVEPAY", "BANK"]:
...
async def export_order_invoice_pdf(
    order_id: str,
    current_user: User = Depends(require_active_subscription()),
    db: Session = Depends(get_db),
):
    ...
    for item in items:
        ...
        data.append([
            ...
            str(item.quantity),
            str(item.price_at_sale),
            str(item.quantity * item.price_at_sale)
        ])

    data.append(["", "", "Total", str(order.total_amount)])

    elements = [
        Paragraph(f"Invoice {order.order_number}", styles["Title"]),
        Paragraph(f"Customer: {order.customer_name}", styles["Normal"]),
        Paragraph(f"Phone: {order.customer_phone}", styles["Normal"]),
    ]
...
async def export_sales_report_pdf(
    ...
    data.append(["", "TOTAL SALES", str(total_sales)])
